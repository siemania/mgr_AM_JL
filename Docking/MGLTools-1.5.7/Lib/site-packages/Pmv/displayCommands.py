...
#
# $Header: /mnt/raid/services/cvs/python/packages/share1.5/Pmv/displayCommands.py,v 1.223.2.5 2016/03/05 00:12:49 annao Exp $
# 
# $Id: displayCommands.py,v 1.223.2.5 2016/03/05 00:12:49 annao Exp $
#
...
"""
        ...
        for mol, atomSets in zip(molecules, ats):
            ...
            for geom in geomList:
                ...
                atoms = mol.geomContainer.atoms.get(geom.name, [])
                if len(atoms)==0:
                    continue
                # handle event.action
                if action=='delete':
        ...
        def drawAtoms( mol, atm, only, negate, scaleFactor, cpkRad, quality,
            ...
            if len(_set)==0: 
                ...
                if propertyName:
                    ...
                    propvals = numpy.array(self.getPropValues(atm, propertyName, propertyLevel), "f")
                    atm.radius = propvals
                atmRad = numpy.array(_set.radius)
...
current selection cannot be colored using this function."
    ...
    def onAddObjectToViewer(self, obj):
        ...
        for atm in obj.allAtoms:
            if not atm.colors.has_key('cpkByProp'):
    ...
    def doit(self, nodes, property, only=False, negate=False, 
        ...
        def drawAtoms( mol, atm, only, negate, 
            ...
            if len(_set)==0: 
                ...
                #cpkSF = numpy.array(_set.cpkScale)
                #atmRad = numpy.array(self.propValues) * scaleFactor
                #cpkRad = numpy.array(self.propValues)
                #cpkRad = numpy.array(_set.cpkRad)
                #sphRad = cpkRad + cpkSF*atmRad
                #sphRad = sphRad.tolist()
                sphRad = numpy.array(self.propValues) * scaleFactor
                g.Set(vertices=_set.coords, inheritMaterial=False, 
                      materials = colors, radii=sphRad, visible=1,
                      quality=quality, tagModified=False)
...
"""
    ...
    def onAddObjectToViewer(self, obj):
        ...
        for atm in obj.allAtoms:
            if not atm.colors.has_key('sticks'):
                atm.colors['sticks'] = (1.0, 1.0, 1.0)
            atm.opacities['sticks'] = 1.0
            if not atm.colors.has_key('balls'):
...
class DisplayBackboneTrace(DisplaySticksAndBalls):
    ...
    def onAddObjectToViewer(self, obj):
        ...
        for atm in obj.allAtoms:
            if not atm.colors.has_key('CAsticks'):
                atm.colors['CAsticks'] = (1.0, 1.0, 1.0)
            atm.opacities['CAsticks'] = 1.0
            if not atm.colors.has_key('CAballs'):
...
##                 for v in coordv:
##                     atom_coords.append(list(numpy.dot(im, v)))
    ...
    def doit(self, nodes, geomNames=None, only=0, negate=0, nbVert=3):
        ...
        for mol, atm  in map(None,molecules,atomSets):
            ...
            for obj in geomNames:
                ...
                nvs = numpy.nonzero(cond) #returns indices of cond,
