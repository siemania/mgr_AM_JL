...
def main():
	...
	for folder, name in zip(args.directories, args.names):
		...
		subprocess.run([
			sys.executable, os.path.join(statistics_folder, "extract_energy_values.py"),
			"-d", folder, "-o", out_file
		], check=True)
	...
	if len(generated_txt_files) >= 2:
		...
		cmd = [
			sys.executable, os.path.join(statistics_folder, "rmsd_histogram.py"),
	...
	# 3. Uruchom energy_correlation.py na wygenerowanych plikach + eksperymentalny
		...
		cmd = [
			sys.executable, os.path.join(statistics_folder, "energy_correlation.py"),
			"-s", generated_txt_files[0],
			"-b", generated_txt_files[1],
			"-f", args.file,
			"-l", args.names[0], args.names[1],
